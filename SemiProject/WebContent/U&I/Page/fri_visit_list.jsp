...
<%
	...
	System.out.println("fri_visit_list==>fri_getSex2==>" + fri_sex);
	String session_sex = (String)session.getAttribute("session_sex");
	System.out.println("fri_visit_list==>getSex3==>" + session_sex);
	FriendBean friendBean = new FriendBean();
	
	//성별 조건 (이미지 가져오기) 친구회원
	if(fri_sex.equals("남자") || fri_sex.equals("male")){
		fri_sex = "1.png";
	}else if(fri_sex.equals("여자") || fri_sex.equals("female")){
		fri_sex = "2.png";
	}
	//세션된 아이디 조건
	if(session_sex.equals("남자") || session_sex.equals("male")){
		session_sex = "1.png";
	}else if(session_sex.equals("여자") || session_sex.equals("female")){
		session_sex = "2.png";
	}
	
	
	session.setAttribute("sex",fri_sex);
			...
			<ul class="sidebar-menu" id="nav-accordion">
				<p class="centered">
					<a href="./New_MyPageView.sr"> <img
						src="./upload/<%=new_fr_PhotoName %>" class="img-circle"
	...
	<section id="main-content">
		<section class="wrapper" style="overflow-y: scroll; height:100%">
			<div class="row" style="margin: 3%">
				...
				<hr>
			<form action="./FriendVisitAddAction.sr" method="post" name="DataInsert">
						...
						<table>
							...
							<tr onmouseover="this.style.backgroundColor='#F8F8F8'"
								...
								<td colspan=2 class=th_width><img width=100px
									src="./U&I/image/<%=session_sex%>"></td>
								<td class='td_width'>
									<% if(session.getAttribute("id").equals(b1.getV_ID())){ %> <%--세션된 아이디와 리스트의 아이디 비교해서 같으면 수정삭제 가능 --%>
										<a href="./FriendVisitDetailAction.sr?num=<%=b1.getV_NUM()%>">
									<%} %>
									<%=b1.getV_CONTENT()%></a>
								</td>
							</tr>
